...
class MathQuiz:
    def __init__(self, root):
        ...
        # ---- Load images and icon ----
        self.menu_bg_path = "menu_bg.jpg"
        self.quiz_bg_path = "quiz_bg.jpg"
        self.icon_path = "quiz_icon.ico"
    ...
    def set_background(self, frame, image_path):
        """Set a static full-size background for a frame"""
        ...
        bg_label.image = photo  # Prevent garbage collection
    ...
    def show_menu(self):
        ...
        self.menu_bg_img = self.set_background(self.menu_frame, self.menu_bg_path)

        tk.Label(self.menu_frame, text="🧮 MATH QUIZ 🧮",
                 font=("Comic Sans MS", 36, "bold"), fg="#4a90e2", bg="#ffffff").pack(pady=30)
        tk.Label(self.menu_frame, text="Select Difficulty Level",
                 font=("Comic Sans MS", 22), fg="#555", bg="#ffffff").pack(pady=10)

        # Adjusted button size so Advanced won't get cut
        for lvl, color in [("Easy","#a8e6cf"),("Intermediate","#fff59d"),("Advanced","#ff8b94")]:
            tk.Button(self.menu_frame, text=lvl, width=20, height=2,  # Adjusted height
                      font=("Comic Sans MS", 18, "bold"), bg=color,
                      command=lambda l=lvl.lower(): self.start_quiz(l)).pack(pady=12)
        
        tk.Label(self.menu_frame, text=f"Each quiz has {self.total_questions} questions",
                 font=("Comic Sans MS", 14), fg="#555", bg="#ffffff").pack(side="bottom", pady=20)
    ...
    def show_question(self):
        ...
        self.quiz_bg_img = self.set_background(self.quiz_frame, self.quiz_bg_path)
    ...
    def check_answer(self):
        ...
        if ans == self.correct_answer:
            ...
            self.score += points
            color = "#4caf50" if self.attempt == 1 else "#fff59d"
    ...
    def show_results(self):
        ...
        self.quiz_bg_img = self.set_background(self.result_frame, self.quiz_bg_path)
